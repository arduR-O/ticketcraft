 ...
 * 
 * Handles event search, seat map generation, and event details.
 * 
 * Placed behind the Gateway but does not require strict authentication (public routes),
...
public class CatalogController {
   ...
   * 
   * Accepts a search query and a page number, returning a list of events.
   * 
   * Uses pagination to handle high traffic efficiently and avoid OutOfMemory errors
   ...
   * 
   * Accepts lat/lng and a page number, returning a list of events ordered by distance.
   * 
   ...
   * 
   * Retrieves all seats associated with the given event ID.
   * 
   * Clients call this once when loading the event page to build the initial SVG or Canvas
   ...
   * 
   * Retrieves the event metadata along with aggregated seat counts and pricing tiers.
   * 
   * Prevents the client from needing to download the entire seatmap just to render the
